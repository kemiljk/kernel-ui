...
/** React wrapper over `observeLineFit`. Writes the attribute straight to the
 * DOM rather than through state: the answer is derived from layout, and routing
 * it through a re-render would re-run layout to learn what layout just said. */
export function useLineFit<T extends HTMLElement = HTMLElement>(): RefObject<T | null> {
  const ref = useRef<T | null>(null);

  useEffect(() => {
    const element = ref.current;
    if (!element) return;
    return observeLineFit(element);
  });
